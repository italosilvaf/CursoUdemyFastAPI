...
async def post_artigo(artigo: ArtigoSchema, usuario_logado: UsuarioModel = Depends(get_current_user), db: AsyncSession = Depends(get_sesion)):
...
async def get_artigos(db: AsyncSession = Depends(get_sesion)):
...
async def get_artigo(artigo_id: int, db: AsyncSession = Depends(get_sesion)):
    async with db as session:
...
async def put_artigo(artigo_id: int, artigo: ArtigoSchema, db: AsyncSession = Depends(get_sesion), usuario_logado: UsuarioModel = Depends(get_current_user)):
    async with db as session:
        query = select(ArtigoModel).filter(ArtigoModel.id == artigo_id)
        result = await session.execute(query)
...
async def delete_artigo(artigo_id: int, db: AsyncSession = Depends(get_sesion), usuario_logado: UsuarioModel = Depends(get_current_user)):
    async with db as session:
